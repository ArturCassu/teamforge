...
import { CATEGORY_LABELS, CATEGORY_COLORS, type SkillCategory } from '@/lib/types';
...
function getSkillCategory(id: string): SkillCategory {
  return SKILLS.find((s) => s.id === id)?.category ?? 'comunicacao';
}

function getTeamScoresMap(team: TeamData): Record<string, number> {
...
function calculateCoverage(team: TeamData): number {
  let covered = 0;
  for (const skill of SKILLS) {
    const max = Math.max(
...
function calculateComplementarity(team: TeamData): number {
...
export default function AdminPage() {
  ...
  const handleDeletePerson = async (personId: string) => {
    ...
    try {
      await apiRemovePerson(code, personId);
      await fetchRoom();
    } catch (err) {
  ...
  const handleBuildTeams = async () => {
    ...
    setBuilding(true);
    try {
      // Update teamSize first if changed
      if (teamSize !== room.teamSize) {
        await updateRoom(code, { teamSize });
      }
      await apiBuildTeams(code);
      await fetchRoom();
      setActiveTab('teams');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro ao montar times');
    } finally {
      setBuilding(false);
    }
  };
  ...
  const handleClearTeams = async () => {
    setClearing(true);
    try {
      await apiClearTeams(code);
      await fetchRoom();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro ao limpar times');
    } finally {
      setClearing(false);
    }
  };
  ...
  const handleReorganize = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao reorganizar');
    } finally {
      setBuilding(false);
    }
  };
  ...
  const handleDrop = async (e: React.DragEvent, targetTeamId: string) => {
    ...
    // Don't move if already in this team
    const team = room?.teams.find((t) => t.id === targetTeamId);
    if (team?.members.some((m) => m.id === personId)) {
    ...
    setMoving(true);
    try {
      await apiMovePerson(code, personId, targetTeamId);
      await fetchRoom();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro ao mover pessoa');
    } finally {
      setMoving(false);
      setDragPersonId(null);
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl bg-zinc-900 border border-zinc-800 p-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700
              text-sm text-white font-medium transition-colors cursor-pointer border border-zinc-700"
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium
              transition-colors cursor-pointer disabled:opacity-50 border ${
              room.status === 'OPEN'
          ...
          {people.length === 0 ? (
            <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-12 text-center space-y-3">
              ...
              <p className="text-zinc-500 text-sm">
                Compartilhe o link acima para que as pessoas preencham suas skills
              </p>
            </div>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {people.map((person) => {
                ...
                return (
                  <div
                    key={person.id}
                    className="rounded-xl bg-zinc-900 border border-zinc-800 p-4 space-y-3 animate-fade-in"
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-white font-semibold">{person.name}</p>
                        <p className="text-zinc-500 text-xs">
                          via {person.addedVia === 'ocr' ? '📷 OCR' : '📝 Manual'}
                        </p>
                      </div>
                      <button
                        type="button"
                        ...
                        className="text-zinc-500 hover:text-red-400 transition-colors cursor-pointer
                          disabled:opacity-50 p-1 rounded-lg hover:bg-red-500/10"
                        title="Remover"
                      >
                        {deleting === person.id ? (
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {top.map((s) => (
                          <span
                            ...
                            style={{
                              backgroundColor: CATEGORY_COLORS[getSkillCategory(s.skillId)] + '20',
                              color: CATEGORY_COLORS[getSkillCategory(s.skillId)],
                            }}
                          >
          ...
          {people.length >= 2 && (
            <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-5 space-y-4">
              ...
              <button
                ...
                disabled={building}
                className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500
        ...
        <div className="space-y-6">
          {teams.length === 0 ? (
            <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-12 text-center space-y-3">
              ...
              <p className="text-zinc-500 text-sm">
                Vá para a aba &ldquo;Pessoas&rdquo; e clique em &ldquo;Montar Times&rdquo;
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <button
                  ...
                  disabled={building}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600/15
                    hover:bg-emerald-600/25 text-emerald-400 text-sm font-medium transition-colors
                    cursor-pointer disabled:opacity-50 border border-emerald-600/30"
                >
                  🔀 Reorganizar
                </button>
                <button
                  ...
                  disabled={clearing}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-red-600/10
                    hover:bg-red-600/20 text-red-400 text-sm font-medium transition-colors
                    cursor-pointer disabled:opacity-50 border border-red-600/20"
                >
                  🗑 Limpar Times
                </button>
                {moving && (
              ...
              <p className="text-zinc-500 text-sm">
                💡 Arraste membros entre times para ajustar manualmente
              </p>
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                {teams.map((team) => {
                  ...
                  const scoresMap = getTeamScoresMap(team);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex justify-center">
                        <RadarChart scores={scoresMap} size={220} color="#10B981" />
                      </div>
                        ...
                        {team.members.map((member) => {
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center gap-3 rounded-xl px-3 py-2 cursor-grab active:cursor-grabbing
                                transition-all select-none ${
                                isDragging
                                  ? 'opacity-40 scale-95 bg-zinc-800'
                              ...
                              <div className="flex gap-1">
                                {top.map((s) => (
                                  <span
                                    ...
                                    style={{
                                      backgroundColor: CATEGORY_COLORS[getSkillCategory(s.skillId)] + '15',
                                      color: CATEGORY_COLORS[getSkillCategory(s.skillId)],
                                    }}
                                    title={getSkillName(s.skillId)}
                                  >
                                    {getSkillName(s.skillId).slice(0, 8)}
                                  </span>
                                ))}
                              </div>
                            </div>
                        ...
                        {(Object.entries(
                          (() => {
                            ...
                            for (const cat of categories) {
                              ...
                              const vals = catSkills.map((sk) => scoresMap[sk.id] ?? 0);
                              catAvg[cat] = vals.reduce((a, b) => a + b, 0) / vals.length;
                            }
                            return catAvg;
                          })(),
                        ) as [SkillCategory, number][])
                          .sort((a, b) => b[1] - a[1])
                          .slice(0, 3)
                          .map(([cat, avg]) => (
                            <span
                            ...
                            >
                              {CATEGORY_LABELS[cat]} {avg.toFixed(1)}
                            </span>
                          ))}
                      </div>
                    </div>
